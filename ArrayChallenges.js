...
function alwaysHungry(arr){
    ...
    for (var i = 0; i < arr.length; i++){
        ...
        if (arr[i] != "food" && arr[i] > arr.length){
...
alwaysHungry([3.14, "food", "pie", true, "food"]); //should say yummy yummy

alwaysHungry([4, 1, 5, 7, 2]); //should say "I'm hungry"
...
function fibonacciArray(x) {
    // the [0, 1] are the starting values of the array to calculate the rest from
    var fibArr = [0, 1];

    while (fibArr.length < x){
        var prev = fibArr[fibArr.length-1];
        var secondprev = fibArr[fibArr.length -2];
        fibArr.push(prev+ secondprev);
    }
